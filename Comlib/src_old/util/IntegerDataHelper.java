...
public class IntegerDataHelper
{
	...
	public boolean registerData(String name, int start, int size)
	{
		...
		if(!this.canRegisterData(start, size)) return false;

        int filter = getFilter(start, size);
		int[] datas = { start, filter};
		
		this.dataMap.put(name, datas);
	...
	/*public void setUseBit(int start, int size)
	{
		this.setUseBit(getFilter(start, size));
	}*/
	
	public void setUseBit(int filter)
	{
		this.useBitFlag = this.useBitFlag | filter;
		
		if(this.useBitFlag == 0xFFFFFFFF) this.canRegister = Boolean.FALSE;
	}
